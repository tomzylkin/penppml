...
```{r try plugin lasso, results = FALSE}
set.seed(123)
bs1 <- bootstrap(data=trade3, dep="export", cluster_id="alt_id2", fixed=list(c("exp", "time"), c("imp", "time"), c("exp", "imp")), indep=7:22, bootreps=10, colcheck_x = TRUE, colcheck_x_fes = TRUE, boot_threshold = 0.01, post=TRUE, gamma_val=0.01, verbose=FALSE)


bootreps =3
uniq_clus <- levels(factor(clus)) # Unique clusters, as characters
save_betas     <- matrix(nrow = ncol(trade3[,7:22]), ncol = bootreps, dimnames = list(colnames(trade3)[7:22],1:bootreps))
save_betas_pre <- matrix(nrow = ncol(trade3[,7:22]), ncol = bootreps, dimnames = list(colnames(trade3)[7:22],1:bootreps))
save_phis      <- matrix(nrow = ncol(trade3[,7:22]), ncol = bootreps, dimnames = list(colnames(trade3)[7:22],1:bootreps))
is_included    <- matrix(nrow = ncol(trade3[,7:22]), ncol = bootreps, dimnames = list(colnames(trade3)[7:22],1:bootreps))
...
for (b in 1:bootreps) {
  ...
  boot_data  <- merge(cbind(clus,trade3), draw, "clus", all.x=FALSE, all.y=TRUE) # Merge (cluster, data) and draw, keep only those drawn, i.e. those that appear in draw
  ...
  boot_exp  <- factor(boot_data$exp)
  boot_imp  <- factor(boot_data$imp)
  boot_time <- factor(boot_data$time)

  boot_rep  <- factor(boot_data$rep)
  boot_exp_time <- interaction(boot_exp, boot_time)
  boot_imp_time <- interaction(boot_imp, boot_time)
  boot_pair     <- interaction(boot_exp, boot_imp)
  ...
  boot_clus    <- boot_data$alt_id2
  boot_clus    <- interaction(boot_clus, boot_rep)
  boot_data$clus2 <- boot_clus

  boot_fes      <- list(boot_exp_time,boot_imp_time,boot_pair)
  boot_trade    <- boot_data$export
  boot_x        <- boot_data[,8:23]

  #boot_mu       <- boot_data$mu
  boot_rta      <- ((rowSums(boot_x ))>0)*1
  boot_data$rta <- boot_rta
  boot_rta_only <- hdfeppml(dep="export", indep="rta",                  fixed = list(c("exp", "time"), c("imp", "time"), c("exp", "imp")),
  tol=1e-6,hdfetol=1e-2,cluster="clus2", data=boot_data)
  boot_mu <- boot_rta_only$mu

  # This runs bootstrap repetition of plugin
  plugin_boot <- mlfitppml(dep="export",indep=8:23,fixed = list(c("exp", "time"), c("imp", "time"), c("exp", "imp")), tol=1e-6,hdfetol=1e-2,cluster="clus2",method="plugin", colcheck_x=FALSE, mu=boot_mu, data=boot_data) # add option for orig.
...
myCluster <- makeCluster(nr_cores, # number of cores to use
                         type = "PSOCK") # type of cluster
registerDoParallel(myCluster)

tic()
set.seed(123)
draws2 <- foreach (i = 1:bootreps, .combine=cbind) %do% {     sort(sample(uniq_clus,replace=TRUE))}

# Run bootstrap repetitions
output2 <- foreach(i = 1:bootreps) %do% {
 boot_one(b=i)}
toc()
# 68 sec
stopCluster(myCluster)

boot_one <- function(b){ draw  <- draws2[,b] # Take one draw
  draw  <- data.table(draw)[,.N,by=draw] # Object containing cluster number and times it drawn
  colnames(draw)[1] <- "clus" # First column contains cluster IDs
  boot_data  <- merge(cbind(clus,trade3), draw, "clus", all.x=FALSE, all.y=TRUE) # Merge (cluster, data) and draw, keep only those drawn, i.e. those that appear in draw
  boot_index <-  rep(row.names(boot_data), boot_data$N) # Repeat row name the times indicated in draw
  ...
  # This runs bootstrap repetition of plugin
  plugin_boot <- mlfitppml(dep="export",indep=8:23,fixed = list(c("exp", "time"), c("imp", "time"), c("exp", "imp")), tol=1e-6,hdfetol=1e-2,cluster="clus2",method="plugin", colcheck_x=FALSE, mu=boot_mu, data=boot_data) # add option for orig.
  output3 <- list(plugin_boot$beta, plugin_boot$beta_pre, plugin_boot$phi, t(colSums(boot_data[,rownames(plugin_boot$beta)]) !=0))
}

save_betas <- matrix(unlist(lapply(output2, `[[`, 1)), ncol=bootreps) 
rownames(save_betas) <- rownames(output2[[1]][[1]])
save_betas_pre <- matrix(unlist(lapply(output2, `[[`, 2)), ncol=bootreps) 
rownames(save_betas_pre) <- rownames(output2[[1]][[2]])
save_phis <- matrix(unlist(lapply(output2, `[[`, 3)), ncol=bootreps) 
rownames(save_phis) <- rownames(output2[[1]][[3]])
is_included <- matrix(unlist(lapply(output2, `[[`, 4)), ncol=bootreps) 
is_included[is.na(is_included[,b]),b] <- FALSE
rownames(is_included) <- rownames(output2[[1]][[4]])

toc()

which(rowSums(save_betas!=0)/bootreps>0.01)
which(reg6$beta!=0)
...
for(bs_i in 1:3) {
  tryCatch({
    clu_inds <- unique(alt_id2)
    which()
    bs_sample_ind <- sort(sample(1:dim(trade2)[1], dim(trade2)[1], replace=TRUE))
    data_bs <- trade2[bs_sample_ind,]
    reg_bs <- mlfitppml(data = data_bs,
                  dep = "export",
                  fixed = list(c("exp", "time"), 
                              c("imp", "time"),
                              c("exp", "imp")),
                  ...
                  method = "plugin",
                  cluster = c("exp", "imp"))
    selected_vars <- c(selected_vars, which(reg_bs$beta != 0))
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}

selected_bs <- NULL
plugin_bs <- function(df,i){
    data_bs <- df[i,]
    reg_bs <- mlfitppml(data = data_bs,
                  dep = "export",
                  fixed = list(c("exp", "time"), 
                  ...
                  method = "plugin",
                  cluster = c("exp", "imp"))
       selected_bs <- c(selected_bs, which(reg_bs$beta!=0))
#    selected_vars <- c(selected_vars, which(reg_bs$beta != 0))
}

tic()
boot_plugin <- boot(trade2, plugin_bs, R=2, parallel="snow")
toc()
```
