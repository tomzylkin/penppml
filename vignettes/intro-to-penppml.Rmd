...
As we can see, the function is designed to be data-frame-friendly: the user sets a data frame of reference in the `data` argument and then picks the dependent, independent and fixed effect variables either by name or by column number within the provided data frame. As explained in the note, if `indep` is empty, the function uses all remaining columns in the data frame by default. Internally, the function will do the necessary transformations of the columns into vectors and matrices, as needed by the algorithm. 
...
reg2 <- mlfitppml(data = trade2,
                  ...
                  penalty = "lasso",
                  lambdas = lambdas,
                  post = FALSE) # This disables the post-lasso regression step, which is optional.
```

The function returns a list that contains a matrix with the coefficient estimates for each value of lambda (in the `beta` element). We can plot the regularization path as follows:

```{r plot regularization path 1}
results <- as.data.frame(reg2$beta)
...
If the user wishes to fit the model for a single value of lambda, they can either use `mlfitppml` as described above (just setting `lambdas == x`, where `x` is a number) or use the `penhdfeppml` function, upon which `mlfitppml` is built, directly. For instance:
...
We can easily check that the coefficient estimates of `penhdfeppml` and `mlfitppml` are equal for a lambda value of 0.005 (within a numerical tolerance):

```{r check penhdfeppml results}
all.equal(as.vector(reg3$beta[!is.na(reg3$beta)]), as.vector(reg2$beta[, 5]), tol = 1e-07)
```
...
cross_ids <- merge(trade[(trade$exp %in% selected) & (trade$imp %in% selected), 5, drop = FALSE], unique_ids, by = "id", all.x = TRUE)
```

Now we activate the cross-validation option in `mlfitppml` and input the ID vector:

```{r try cross validation, results = FALSE, eval = FALSE}
...
Now the function also returns a `rmse` element that includes the cross-validation results (the average RMSE or root mean squared error for each value of lambda). Users can employ this tool to choose a value for the penalty parameter:
...
We can also graph the relationship between the penalty parameter and the RMSE:

```{r graph xval results, echo = FALSE}
ggplot2::ggplot(data = rmse, mapping = ggplot2::aes(x = lambda, y = rmse)) +
  ggplot2::geom_line(ggplot2::aes(col = "red"), show.legend = FALSE) +
  ggplot2::scale_x_reverse() +
  ggplot2::labs(x = "Penalty parameter (lambda)", y = "RMSE", title = "Figure 2: Cross-validation Results") +
    ggplot2::theme_classic()
```
For instance, this suggest that the penalty should be set at 0 in this case. In other words, the results in Table 3 are the ones that minimize the mean squared error of the regression, according to 20-fold cross-validation.

### Plugin lasso

This package also enables the use of the plugin lasso method, incorporating coefficient-specific penalty weights calculated automatically by the package. The most convenient way to do this is to set `method = "plugin"` in `mlfitppml`. Note that a clustering variable is required by the plugin algorithm:
...
results <- data.frame(prov = rownames(reg6$beta), b = reg6$beta, se = 0)
...
knitr::kable(iceberg_results, 
             format = "pipe",
             caption = "Table 6: Iceberg Lasso results",
             row.names = TRUE,
             digits = 4)
```
...
Correia, S., P. Guimaraes and T. Zylkin (2020). "Fast Poisson estimation with high dimensional fixed effects", \emph{STATA Journal}, 20, 90-115.

Gaure, S (2013). "OLS with multiple high dimensional category variables", \emph{Computational Statistics & Data Analysis}, 66, 8-18.

Friedman, J., T. Hastie, and R. Tibshirani (2010). "Regularization paths for generalized linear models via coordinate descent", \emph{Journal of Statistical Software}, 33, 1-22.
