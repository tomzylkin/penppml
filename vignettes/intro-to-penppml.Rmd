...
We can see that the plugin lasso is very strict: only four provisions have non-zero coefficients when the penalties are set at the level suggested by the plugin algorithm. Compare this to the cross-validation results, which didn't remove any of the provisions.
...
When doing this, we see that one more provision than before is selected.
